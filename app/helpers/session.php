...
class Session {
	...
	private static $_sessionSalt = "54xZGbojMRDmsSnf022mCC9H";
	...
	public static function setEncrypt($key,$value = false){
		...
		*/
		if(is_array($key) && $value === false){
			foreach ($key as $name => $value) {
				$value = base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5(self::$_sessionSalt), $value, MCRYPT_MODE_CBC, md5(md5(self::$_sessionSalt))));
				$_SESSION[SESSION_PREFIX.$name] = $value;
			}
	...
	public static function getEncrypt($key,$secondkey = false){

		if($secondkey == true){

			if(isset($_SESSION[SESSION_PREFIX.$key][$secondkey])){
				$val =  $_SESSION[SESSION_PREFIX.$key][$secondkey];
				$val = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5(self::$_sessionSalt), base64_decode($val), MCRYPT_MODE_CBC, md5(md5(self::$_sessionSalt))), "\0");
				return $val;
			}
			...
			if(isset($_SESSION[SESSION_PREFIX.$key])){
				$val =  $_SESSION[SESSION_PREFIX.$key];
				$val = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5(self::$_sessionSalt), base64_decode($val), MCRYPT_MODE_CBC, md5(md5(self::$_sessionSalt))), "\0");
				return $val;
			}
